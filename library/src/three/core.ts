...
export class MiniDRACOLoaderBase extends Loader<BufferGeometry> {
  ...
  _getWorker(): WorkerEntry | null {
    ...
    if (this._workers.length < this.workerLimit) {
      let worker: Worker
      try {
        const created = this._createWorker()
  ...
  _createWorker(): Worker | null {
    if (this._workerUrl === null) return null
    return this._createWorkerFromUrl(this._workerUrl)
  }
  ...
  _createWorkerFromUrl(workerUrl: string | URL): Worker | null {
    try {
      ...
      try {
        ...
        return new Worker(this._workerBlobUrl, { type: 'module' })
      } catch {
        this._workersBroken = true
        return null
      }
